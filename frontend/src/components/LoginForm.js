...
function Login() {
  ...
  const handleInitialLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://notepad-2-qtih.onrender.com/api/users/login', {
        email,
        password,
      });
  ...
  const handleOTPVerification = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://notepad-2-qtih.onrender.com/api/users/verify-otp', {
        email,
        otp,
        tempOTP,
      });
